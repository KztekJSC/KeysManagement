...
namespace Kztek_Web.Areas.Admin.Controllers
{
    ...
    public class AppController : Controller
    {
        ...
        public AppController(IAppService _AppService)
        {
        ...
        {
            var gridmodel = await _AppService.GetPagingByFirst(key, page, 20);

            ViewBag.keyValue = key;
            ViewBag.AuthValue = await AuthHelper.CheckAuthAction("App", this.HttpContext);
        ...
        [CheckSessionCookie(AreaConfig.Admin)]
        [HttpGet]
        public async Task<IActionResult> Create(App model, string AreaCode = "", string key = "", string gate = "")
        {

            model = model == null ? new App() : model;
            ViewBag.keyValue = key;
            ViewBag.AreaCodeValue = AreaCode;
        ...
        [CheckSessionCookie(AreaConfig.Admin)]
        [HttpPost]
        public async Task<IActionResult> Create(App model, string key = "", bool SaveAndCountinue = false, string AreaCode = "", string gate = "")
        {
            ...
            var existed = await _AppService.GetByName(model.Name);
            if (existed != null )
            {
            ...
            //Thực hiện thêm mới
            var result = await _AppService.Create(model);
            if (result.isSuccess)
            {
                if (SaveAndCountinue)
                {
        ...
        public async Task<IActionResult> Update(string id, string AreaCode = "", int page = 1, string key = "")
        {
            ...
            var model = await _AppService.GetById(id);
            ViewBag.PN = page;
            ViewBag.AreaCodeValue = AreaCode;
            ViewBag.keyValue = key;
        ...
        [CheckSessionCookie(AreaConfig.Admin)]
        [HttpPost]
        public async Task<IActionResult> Update(App model, string AreaCode = "", int page = 1, string key = "")
        {
            ...
            var oldObj = await _AppService.GetById(model.Id);
            if (oldObj == null)
            {
            ...
            var result = await _AppService.Update(oldObj);
            if (result.isSuccess)
            {
